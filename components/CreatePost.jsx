    ...
    if (path.includes("update")) {
      try {
        const getPost = await fetch(
          `/api/update?id=${path.split("update/")[1]}`,
          {
            method: "GET",
            headers: {
  ...
  const updateYourPost = async () => {
    try {
      ...
      const updateCall = await fetch(`/api/post`, {
        ...
        body: JSON.stringify({
          id: path.split("update/")[1],
          gotDescription: desc,
          gotImage: cloudinaryImage,
        }),
      });
      ...
      <div className="CreateContainer w-[70%] bg-yellow-300 p-3 rounded-xl border-4 shadow-2xl shadow-black mt-[1rem] mb-[1.5rem]">
        <form action="" className="flex flex-col gap-3">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button className="bg-black font-bold hover:scale-105 transition-all text-white  cursor-pointer p-1 px-3 rounded-lg">
              Cancel
            </button>
          </div>
        </form>
